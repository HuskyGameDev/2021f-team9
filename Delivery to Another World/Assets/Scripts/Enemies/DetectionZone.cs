...
public class DetectionZone : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Middle ray
            if (Physics.Raycast(new Vector3(transform.position.x, transform.position.y + 0.55f, transform.position.z), transform.forward, out hit[0], 6f, layerMask) && hit[0].collider.CompareTag("Player"))
            {
            ...
            {
                Debug.DrawRay(new Vector3(transform.position.x, transform.position.y + 0.55f, transform.position.z), transform.forward * 6f, Color.white); // Middle
